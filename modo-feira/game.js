(function () {
  ...
  function openScreen(id) {
    ...
    if (heading) {
      ...
      window.setTimeout(function () { heading.focus({ preventScroll: true }); }, 100);
    }
    playTone("move");
  }
  ...
  const requestFull = document.documentElement.requestFullscreen || document.documentElement.webkitRequestFullscreen;
  if (!requestFull) {
  ...
  function renderProgress() {
    ...
    for (let i = 0; i < fossils.length; i += 1) {
      ...
      if (i < discoveries) dot.className = "done";
      scannerProgress.appendChild(dot);
    }
    scannerProgressText.textContent = discoveries + "/" + fossils.length;
  }
  ...
  function renderDeck() {
    ...
    for (let i = 1; i <= 15; i += 1) {
      ...
      card.setAttribute("aria-label", "Escolher carta " + i);
  ...
  function resetScannerRound() {
    ...
    uvButton.style.setProperty("--hold-progress", "0deg");
    uvButton.disabled = true;
    uvButton.querySelector("strong").textContent = "Segure para escanear";
    uvButton.querySelector("small").textContent = "mantenha pressionado";
    emptyCard.hidden = false;
    fossilCard.hidden = true;
    fossilCard.style.backgroundImage = "none";
    fossilCard.setAttribute("aria-label", "Carta ainda não revelada");
    classificationPanel.hidden = true;
    scannerResult.hidden = true;
    scannerResult.className = "answer-box";
    scannerResult.innerHTML = "";
    scannerInstruction.textContent = "Primeiro, escolha uma carta do monte.";
    scanStatus.textContent = "Escolha uma carta para começar.";
    resetChoiceButtons();
  ...
  function selectCard(card) {
    if (scannerRevealed || scannerAnswered || card.classList.contains("is-used")) return;
    ...
    currentFossil = rounds[roundIndex];
    fossilCard.style.backgroundImage = "url('" + currentFossil.image + "')";
    fossilCard.setAttribute("aria-label", "Carta escolhida; fóssil ainda oculto");
    fossilCard.hidden = false;
    emptyCard.hidden = true;
    uvButton.disabled = false;
    scannerInstruction.textContent = "Agora, use a luz UV para investigar.";
    scanStatus.textContent = "Segure o botão UV até o scanner completar a leitura.";
    playTone("move");
    if (navigator.vibrate) navigator.vibrate(25);
  }

  function holdTick(now) {
    if (!holding || scannerRevealed) return;
    const elapsed = now - holdStart;
    const progress = Math.min(elapsed / HOLD_DURATION, 1);
    uvButton.style.setProperty("--hold-progress", (progress * 360) + "deg");
    if (progress >= 1) {
      completeScan();
      return;
    }
  ...
  function beginHold(event) {
    if (uvButton.disabled || scannerRevealed || holding) return;
    if (event.type === "keydown" && event.key !== " " && event.key !== "Enter") return;
    if (event.type === "keydown") event.preventDefault();
    if (event.pointerId !== undefined && uvButton.setPointerCapture) {
      try { uvButton.setPointerCapture(event.pointerId); } catch (error) { /* Sem suporte completo a captura. */ }
    }
    holding = true;
    holdStart = performance.now();
    uvButton.classList.add("is-holding");
    uvDevice.classList.add("is-scanning");
    scanStatus.textContent = "Continue segurando... a luz UV está varrendo a carta.";
    holdFrame = window.requestAnimationFrame(holdTick);
  }
  ...
  function cancelHold() {
    ...
    if (!scannerRevealed) {
      ...
      uvButton.style.setProperty("--hold-progress", "0deg");
      scanStatus.textContent = "Quase! Mantenha o botão pressionado até completar o círculo.";
    }
  }
  ...
  function completeScan() {
    ...
    uvButton.style.setProperty("--hold-progress", "360deg");
    uvButton.disabled = true;
    uvButton.querySelector("strong").textContent = "Fóssil revelado!";
    uvButton.querySelector("small").textContent = "agora classifique a descoberta";
    uvDevice.classList.remove("is-scanning");
    uvDevice.classList.add("is-revealed");
    fossilCard.setAttribute("aria-label", "Fóssil revelado. Observe a imagem e escolha a era correta.");
    scanStatus.textContent = "Leitura concluída. Observe os detalhes e escolha a era.";
    classificationPanel.hidden = false;
    playTone("scan");
    if (navigator.vibrate) navigator.vibrate([40, 40, 80]);
    window.setTimeout(function () {
      classificationPanel.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 380);
  }
  ...
  uvButton.addEventListener("contextmenu", function (event) { event.preventDefault(); });
  ...
  function finishScannerRound() {
    ...
    if (discoveries === fossils.length) {
      scannerResult.innerHTML += '<div class="answer-actions"><button class="primary-button" id="finishScanner" type="button">Concluir missão</button></div>';
      document.getElementById("finishScanner").addEventListener("click", function () {
        classificationPanel.hidden = true;
        scannerCompletion.hidden = false;
        scannerCompletion.scrollIntoView({ behavior: "smooth", block: "center" });
        playTone("correct");
      });
    } else {
      scannerResult.innerHTML += '<div class="answer-actions"><button class="primary-button" id="nextCard" type="button">Escolher próxima carta →</button></div>';
      document.getElementById("nextCard").addEventListener("click", function () {
        roundIndex += 1;
        resetScannerRound();
        document.querySelector(".deck-panel").scrollIntoView({ behavior: "smooth", block: "start" });
      });
    }
  }
  ...
  eraChoices.addEventListener("click", function (event) {
    ...
    if (!button || !scannerRevealed || scannerAnswered) return;
    ...
    if (chosenEra !== currentFossil.era) {
      ...
      const clueIndex = Math.min(wrongAttempts - 1, currentFossil.clues.length - 1);
      scannerResult.className = "answer-box wrong";
      scannerResult.innerHTML = "<h4>Ainda não — tente outra era.</h4><p>" + currentFossil.clues[clueIndex] + "</p>";
      playTone("wrong");
    ...
    eraChoices.querySelectorAll("button").forEach(function (choice) { choice.disabled = true; });
    scannerResult.className = "answer-box correct";
    scannerResult.innerHTML =
      "<h4>Correto! É " + currentFossil.name + ".</h4>" +
      "<p>Ele pertence à era <strong style=\"color:" + currentFossil.color + "\">" + currentFossil.eraName + "</strong>. " + currentFossil.fact + "</p>" +
      '<div class="answer-actions"><a class="secondary-button link-button" href="' + currentFossil.href + '">Explorar esta era</a></div>';
    playTone("correct");
  ...
  restartScanner.addEventListener("click", function () {
    resetScannerGame();
    document.querySelector("#scannerScreen .section-heading").scrollIntoView({ behavior: "smooth", block: "start" });
  });
  ...
  function setRangeVisual(input, value, showNumber) {
    ...
    output.value = showNumber ? Math.round(value) + "%" : "?";
    output.textContent = output.value;
  }

  markerRows.forEach(function (row) {
    const input = row.querySelector("input");
    setRangeVisual(input, Number(input.value), false);
  ...
  function animateMarker(input, from, to, duration) {
    ...
    function frame(now) {
      ...
      setRangeVisual(input, from + (to - from) * eased, true);
  ...
  function scoreTimeline(errors) {
    const average = errors.reduce(function (sum, value) { return sum + value; }, 0) / errors.length;
    if (average <= 3) return "Excelente estimativa! Você chegou muito perto da escala real.";
    if (average <= 8) return "Muito bem! Você percebeu que as eras mais recentes ocupam uma parte pequena da linha.";
  ...
  revealTimeline.addEventListener("click", function () {
    ...
    markerRows.forEach(function (row) {
      ...
      errors.push(Math.abs(guess - targets[era]));
      input.disabled = true;
      animateMarker(input, guess, targets[era], 1150);
    });
    timelineBoard.classList.add("is-revealed");
    revealTimeline.disabled = true;
    revealTimeline.textContent = "Linha verdadeira revelada";
    timelineFeedback.hidden = false;
    timelineFeedback.innerHTML = "<strong>" + scoreTimeline(errors) + "</strong><br>Os marcadores estão se movendo para suas posições corretas.";
    timelineReveal.hidden = false;
    playTone("correct");
    if (navigator.vibrate) navigator.vibrate([45, 40, 80]);
    window.setTimeout(function () {
      timelineReveal.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 1050);
  });
  ...
  revealHumans.addEventListener("click", function () {
    ...
    revealHumans.textContent = "Estamos no último pontinho da linha";
    playTone("human");
    if (navigator.vibrate) navigator.vibrate([30, 50, 30, 50, 90]);
    window.setTimeout(function () {
      humanAnswer.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 250);
  });
  ...
  restartTimeline.addEventListener("click", function () {
    ...
    revealHumans.textContent = "E nós, onde estamos?";
    revealTimeline.disabled = false;
    revealTimeline.textContent = "Revelar linha verdadeira";
    markerRows.forEach(function (row) {
      const input = row.querySelector("input");
      input.disabled = false;
      setRangeVisual(input, initialTimeline[row.dataset.era], false);
    });
    document.querySelector("#timelineScreen .section-heading").scrollIntoView({ behavior: "smooth", block: "start" });
  });
  ...
  if ("serviceWorker" in navigator && (location.protocol === "https:" || location.hostname === "localhost")) {
    window.addEventListener("load", function () {
      navigator.serviceWorker.register("sw.js").catch(function () {
        /* O jogo continua funcionando normalmente mesmo sem cache offline. */
      });
    });
  }
}());
